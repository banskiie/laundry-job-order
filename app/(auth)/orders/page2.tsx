...
const ORDERS_LIST = gql`
  query OrderList($rows: Int, $page: Int) {
    orderList(rows: $rows, page: $page) {
      total
      pages
    }
  }
...
//   console.log(total, nodes, pageInfo)
...
//                 Showing {(page.current - 1) * ROW_COUNT + 1}-
//                 {page.current === page.max ? total : page.current * ROW_COUNT}{" "}
//                 out of {total} result{total === 1 ? "" : "s"} for{" "}
...
//         {(
//           nodes.slice(
//             (page.current - 1) * ROW_COUNT,
//             page.current * ROW_COUNT,
//           ) || []
//         ).map((o: any) => (
